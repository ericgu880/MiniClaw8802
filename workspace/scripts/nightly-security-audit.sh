...
# macOS find 不支持 -mtime -1 的某些用法，用 -mtime 1
...
# macOS 用 launchctl 而不是 systemd
ls -la /etc/cron.* /var/at/tabs/ 2>/dev/null >> "$REPORT_FILE" || true
...
if [ -f "$OC/.config-baseline.sha256" ]; then
  # macOS 用 shasum -a 256 -c
  HASH_RES=$(cd "$OC" && shasum -a 256 -c .config-baseline.sha256 2>&1 || true)
fi
echo "Hash Check: $HASH_RES" >> "$REPORT_FILE"
# macOS stat 用 -f "%Lp" 而不是 -c "%a"
...
PERM_SSHD=$(stat -f "%Lp" /etc/ssh/sshd_config 2>/dev/null || echo "N/A")
PERM_AUTH_KEYS=$(stat -f "%Lp" "$HOME/.ssh/authorized_keys" 2>/dev/null || echo "N/A")
...
if [ -n "$GW_PID" ]; then
  # macOS 没有 /proc，用 ps 和 launchctl
...
if [ -d "$SCAN_ROOT" ]; then
  ...
  # 12/24-word mnemonic-ish
  H2=$(grep -RInE --exclude-dir=.git --exclude='*.png' --exclude='*.jpg' --exclude='*.jpeg' --exclude='*.gif' --exclude='*.webp' '\b([a-z]{3,12}\s+){11}([a-z]{3,12})\b|\b([a-z]{3,12}\s+){23}([a-z]{3,12})\b' "$SCAN_ROOT" 2>/dev/null | wc -l | tr -d ' ')
  DLP_HITS=$((H1 + H2))
fi
